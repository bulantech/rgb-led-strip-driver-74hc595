...
const int ShiftPWM_latchPin=4; //8;
...
{ 
  char buf[32];
  //Serial.println("---------");
  sprintf(buf, "%d, %d, %d, %d, %d \r\n", colorCount, ledCount, COLOR[colorCount][0], COLOR[colorCount][1], COLOR[colorCount][2]);
  Serial.println(buf);
  
  ShiftPWM.SetRGB(ledCount, COLOR[colorCount][0], COLOR[colorCount][1], COLOR[colorCount][2]);    // red
  if(++ledCount >= numRGBleds) {
    ledCount = 0;
    delay(1000);
